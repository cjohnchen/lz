...
R"(
    __kernel void apply_se(
                  ...
                  __constant const float * restrict scales_biases,
                  const int has_se_bias) {
		...
		const int scale_loc = batch * 2 * channels + channel;
		const int bias_loc = scale_loc + channels;

        if (col < BOARD_SIZE) {
            real sig_scale = scales_biases[scale_loc];
            sig_scale = 1.0f/(1.0f + exp(-sig_scale));

            for ( int i = 0; i < BOARD_SIZE; i++) {
                const int idx = c * NUM_INTERSECTIONS + col * BOARD_SIZE + i;
                const real in = vload_net_t(idx, input);
                const real res = vload_net_t(idx, residual);

                real val = sig_scale * in + scales_biases[bias_loc] + res;
