...
UCTNode* UCTNode::uct_select_child(int color, bool is_root) {
    ...
    for (auto& child : m_children) {
        ...
        const auto puct = cfg_puct + cfg_puctscale * std::log((parentvisits + cfg_cbase)/cfg_cbase);
        const auto prior = puct * psa * (numerator / denom);
        const auto value = winrate + prior;
